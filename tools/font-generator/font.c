...
void main()
{
    ...
    size_t texture_size = FPF_TEXTURE_WIDTH*FPF_TEXTURE_HEIGHT;

    texture = (unsigned char *)malloc(sizeof(unsigned char)*texture_size);
    fpf_create_alpha_texture(texture, texture_size, FPF_TEXTURE_WIDTH, FPF_RASTER_Y_AXIS);

    for (unsigned char g=' '; g<='~'; ++g)
    {
        unsigned int x_start;
        unsigned int y_start;
        fpf_get_glyph_position(g, &x_start, &y_start);

        for (int y = 0; y<FPF_GLYPH_HEIGHT-1; ++y)
        {
            printf("\"");
            for (int x = 0; x<FPF_GLYPH_WIDTH-1; ++x)
            {
                unsigned char pixel = texture[(y+y_start)*FPF_TEXTURE_WIDTH+(x+x_start)];
                if (pixel == 0)
                {
                    printf(".");
                } else {
